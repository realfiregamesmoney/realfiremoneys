    ...
    useEffect(() => {
        ...
        const consumeRaceAndInit = async () => {
            ...
            try {
                ...
                console.error("Erro fatal ao iniciar jogo de pulo:", e);
        ...
        return () => {
            ...
            if (finalScore > 0 && !hasQuitRef.current) {
                ...
                if (sessionIdRef.current) {
                    supabase.from('minigame_sessions').update({ status: 'finished', score: finalScore }).eq('id', sessionIdRef.current).then();
                }
            }
        };
    }, [user, navigate]);
