...
console.log("🎮 Papel na arena de Xadrez:", myRole);
...
function startTimers() {
    ...
    gameInterval = setInterval(() => {
        ...
        if (gameTime <= 0) {
            if (capturesP1 === capturesP2) endGame(0, "Empate por tempo!");
            else endGame(capturesP1 > capturesP2 ? 1 : 2, "Tempo da Partida Esgotado!");
        }
        updateGameTimerUI();
...
function updatePlayerClocksUI() {
    ...
    score1El.innerHTML = `Peças: ${16 - capturesP2} <div class="timer-val">${formatTime(p1Time)}</div>`;
    score2El.innerHTML = `Peças: ${16 - capturesP1} <div class="timer-val">${formatTime(p2Time)}</div>`;
}
...
function executeMove(move) {
    ...
    if (targetPiece) {
        ...
        if (turn === 1) {
            capturesP1++;
            if (targetPiece.t === 'K') setTimeout(() => endGame(1, "Você capturou o rei adversário!"), 100);
        } else {
            capturesP2++;
            if (targetPiece.t === 'K') setTimeout(() => endGame(2, "Você perdeu seu rei!"), 100);
        }
    } else {
        playSound('move');
    }
