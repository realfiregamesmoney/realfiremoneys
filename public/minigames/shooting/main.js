...
// LIGHTS
const ambientLight = new THREE.AmbientLight(0x444444); // Brighter ambient
...
fCtx.fillStyle = '#010101';
fCtx.fillRect(0, 0, 128, 128);
fCtx.strokeStyle = '#0c0c0c';
fCtx.lineWidth = 1;
...
const buildingCount = 45; // Reduced for performance
...
bCtx.fillStyle = '#222222'; // darker wall color (but visible)
bCtx.fillRect(0, 0, 256, 256);
bCtx.fillStyle = '#ea580c'; // warm glowing window light (orange)
for (let y = 10; y < 256; y += 35) {
    for (let x = 10; x < 256; x += 25) {
        if (Math.random() > 0.6) { // 40% chance of lit window
            bCtx.shadowBlur = 10;
            bCtx.shadowColor = '#f97316';
...
const ruinMat = new THREE.MeshPhongMaterial({
    color: 0x666666, // Brighter to reflect ambient light
    map: windowTexture,
    flatShading: true,
    shininess: 30
});
const ruinInstancedMesh = new THREE.InstancedMesh(ruinGeo, ruinMat, buildingCount);
ruinInstancedMesh.castShadow = false; // Disabled for heavy city
ruinInstancedMesh.receiveShadow = false;
...
const dustMat = new THREE.PointsMaterial({ color: 0xea580c, size: 0.15, transparent: true, opacity: 0.6 });
...
window.addEventListener('touchstart', (e) => {
    /* Atalho de reinício via toque removido para forçar cobrança via botão */
    isTouching = true;
...
/* Removido para forçar cobrança de passe via TRY_AGAIN_REQUEST */
...
function shootLaser() {
    ...
    const fireSingle = (offsetX) => {
        ...
        let hexColor = 0xf97316; // orange
        if (weaponLevel === 2) hexColor = 0xfacc15; // yellow
        if (weaponLevel >= 3) hexColor = 0xff0000; // red
...
if (__restartBtn) {
    __restartBtn.addEventListener('click', (e) => {
        console.log("[DEBUG] Clique no botão de reinício dentro do iframe.");
        e.stopPropagation(); // prevent quit game
        if (window.parent && window.parent !== window) {
            console.log("[DEBUG] Solicitando cobrança de passe (TRY_AGAIN_REQUEST) ao app principal...");
