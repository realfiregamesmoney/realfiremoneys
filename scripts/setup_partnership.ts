...
async function setupPartnershipConfig() {
    ...
    const defaultAffiliates = [
        {
            "id": "aff_1",
            "title": "Produto Digital 1",
            "url": "https://hotmart.com/...",
            "description": "Ganhe 50% de comissão divulgando isso."
        }
    ];

    const ops = [
        supabase.from('app_settings').upsert({ key: 'parceria_cutoff', value: '10' }),
        supabase.from('app_settings').upsert({ key: 'parceria_whatsapp', value: '+5511999999999' }),
        supabase.from('app_settings').upsert({ key: 'parceria_questions', value: JSON.stringify(defaultQuestions) }),
        supabase.from('app_settings').upsert({ key: 'parceria_affiliates', value: JSON.stringify(defaultAffiliates) })
    ];
