...
async function run() {
  await supabase.from('app_settings').upsert({ key: 'parceria_cutoff', value: '10' });
  await supabase.from('app_settings').upsert({ key: 'parceria_whatsapp', value: '+5511999999999' });
  await supabase.from('app_settings').upsert({ key: 'parceria_questions', value: JSON.stringify(qs) });
}
run();
